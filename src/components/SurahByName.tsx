...
const SurahByName = () => {
    const [filter, setFilter] = useState<Surah[] | null>()
    const [surahs, setSurahs] = useState<S[] | undefined>(undefined)
    const { setValue, value } = useContext(SearchContext)

    useEffect(() => {

        setFilter(surahs?.filter(el => el?.name?.includes(value)))
    }, [value])
    console.log(quran);
